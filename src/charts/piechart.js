...
const PieChart = ({ chartElName, crustData, data, name, onSliceClick, options }) => {
  const chartOptions = {
    ...options,
    ...defaultOptions,
    series: [
  ...
  return <HighchartsReact id={chartElName} highcharts={Highcharts} options={chartOptions} />;
};
...
const ChartClickModal = ({ anchorEl, data, handleClose, id, open }) => {
  return (
    <Popover
      id={id}
      open={open}
...
const PieChartWithLegend = ({ crustData, data, name, value }) => {
  const [legends] = React.useState(
    crustData ? [...data, ...crustData.filter(({ hiddenLegend }) => !hiddenLegend)] : [...data]
  );
