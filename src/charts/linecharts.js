...
const defineEraRanges = ({ data }) => {
  ...
  return { minAxisX: minX * 0.3, maxAxisX: maxX * 1.02, pixelIntervalX };
};
...
const highOptions = (title, data, tooltipFormatter, labelsFormatter, maxY) => {
  ...
  const { maxAxisX, minAxisX } = defineEraRanges(data);
  const options = {
    title: {
      text: title,
      align: 'left',
    ...
    case 'high':
      return (
        <HighchartsReact
          ...
          options={highOptions(title, data, tooltipFormatter, labelsFormatter, maxY)}
        />
      );
    default:
