...
const ExtrinsicComponent = () => {
  ...
  const [blockNumber, extrinsicIndex] = !isHash
    ? extrinsicIdOrHash.split('-')
    : [];

  const where = useMemo(() => isHash
  ...
  if (error) {
    return (
      <Container sx={{ my: 5 }}><Error /></Container>
    );
  }
  ...
  return (
    <Container sx={{ my: 5 }}>
      <Breadcrumb />
      <Box sx={{ mb: 5 }}>
        <Typography variant='h1'>Extrinsic #{extrinsic.blockNumber}-{extrinsic.extrinsicIndex}</Typography>
      </Box>
      ...
      <Box sx={{ mt: 2 }}>
        {loading ? (
          ...
          extrinsic.hash && (
            <ExtrinsicEventsTabs blockNumber={extrinsic.blockNumber} />
          )
        )}
      </Box>
    </Container>
