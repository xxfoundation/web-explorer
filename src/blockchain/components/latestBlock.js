...
const BlockStatusToIcon = (status, duration) => {
  ...
  const { icon, label } = statusToIconMap[status];
  return (
      ...
      <Grid item xs>
        ...
        <Box sx={{ mt: 1 }}>
          <Link href="" underline="hover" variant="body3">
            {currentData.instrinsic} instrinsic
          </Link>{' '}
          <Typography variant="body3">|</Typography>{' '}
          <Link href="" underline="hover" variant="body3">
            {currentData.events} event
          </Link>
        </Box>
      </Grid>
      <Grid item xs="auto">
        {BlockStatusToIcon(currentData.status, currentData.duration)}
      </Grid>
    </Grid>
  );
};
