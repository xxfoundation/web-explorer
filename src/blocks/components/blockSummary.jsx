...
const BlockSummary = ({number, data}) => {
    return <Grid container spacing={2}>
        ...
        <Grid item xs={12}>
            <SummaryPaper data={[
                ["time", data.time],
                ["status", <><CheckCircleOutlineIcon color={theme.palette.success.main}/>{data.status}</>],
                ["era", data.era],
                ["hash", textWithCopy(data.hash, <Typography>{data.hash}</Typography>)],
                ["parent hash", backAndForwardWithLabel(data)],
                ["state root", <><CheckCircleOutlineIcon color={theme.palette.success.main} />{data.stateRoot}</>],
